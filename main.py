...
def check_expiry():
	...
	for row in data:
		date_object = datetime.strptime(row[4], '%Y-%m-%d').date()
...
def remove_record():
   if not tree.selection():
       ...
       connector.execute('DELETE FROM Product WHERE Product_ID=%d' % selection[0])
       connector.commit()
...
Button(left_frame, text='Add Record', font=labelfont, command=add_record, width=10).place(relx=0.025, rely=0.35)
Button(left_frame, text='View Record', font=labelfont, command=view_record, width=10).place(relx=0.500, rely=0.35)
Button(left_frame, text='Reset Fields', font=labelfont, command=reset_fields, width=10).place(relx=0.025, rely=0.40)
Button(left_frame, text='Delete database', font=labelfont, command=reset_form, width=15).place(relx=0.500, rely=0.40)
Button(left_frame, text='Delete Record', font=labelfont, command=remove_record, width=10).place(relx=0.025, rely=0.45)
Button(left_frame, text='Check Expiry of product', font=labelfont, command=check_expiry, width=18).place(relx=0.025, rely=0.50)

# Placing components in the center frame


# Placing components in the right frame
Label(right_frame, text='Product Records', font=headlabelfont, bg='DarkGreen', fg='LightCyan').pack(side=TOP, fill=X)
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Product ID', "Name", "Type", "Date of Purchase", "Date of Expiry"))
X_scroller = Scrollbar(tree, orient=HORIZONTAL, command=tree.xview)
Y_scroller = Scrollbar(tree, orient=VERTICAL, command=tree.yview)
X_scroller.pack(side=BOTTOM, fill=X)
Y_scroller.pack(side=RIGHT, fill=Y)
tree.config(yscrollcommand=Y_scroller.set, xscrollcommand=X_scroller.set)
tree.heading('Product ID', text='ID', anchor=CENTER)
